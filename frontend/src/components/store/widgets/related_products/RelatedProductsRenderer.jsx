...
// Update export to include isCustomDomain
export const RelatedProductsRenderer = ({ style, content, productId, product, storeId, isEditor, storeDiscounts, isCustomDomain }) => {
    // ...
    // ...
    // Pass isCustomDomain to ProductCard
    <ProductCard
        p={p}
        showPrice={showPrice}
...
const ProductCard = ({ p, showPrice, showDiscount, showRating, showDescription, isEditor, storeId, storeDiscounts, isCustomDomain }) => {
    // Determine wrapper
    ...
    return (
        <Wrapper
            to={!isEditor && !isMock ? (isCustomDomain ? `/p/${p.id}` : `/s/${storeId ? 'store' : 'preview'}/p/${p.id}`) : undefined}
            // Note: We need storeSubUrl for non-custom domain. storeId is available but subUrl isn't directly passed to ProductCard usually.
            // However, RelatedProductsRenderer doesn't seem to have access to storeSubUrl easily unless we pass "store" object.
            // Wait, "storeId" is passed. We might need to stick to relative if we don't have subUrl, OR update to pass store object.
            // Actually, `../${p.id}` was working for subUrl.
            // For custom domain: `/p/${p.id}` is absolute and safe.
            // For subUrl: `../${p.id}` is safe if we are at `.../p/xyz`.
            // Let's rely on relative for subUrl case (existing behavior) and absolute for custom domain?
            // Or better: pass `store` object to RelatedProductsRenderer.

            // Let's stick to relative for non-custom, and absolute for custom.
            // If isCustomDomain, to=`/p/${p.id}`.
            // Else, to=`../${p.id}`.

            to={!isEditor && !isMock ? (isCustomDomain ? `/p/${p.id}` : `../${p.id}`) : undefined}
            relative={isCustomDomain ? undefined : "path"}
