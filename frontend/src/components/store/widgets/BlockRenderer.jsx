...
export function BlockRenderer({ id, type, settings, viewMode, store, products, product, categories, categorySlug, parentSlug, childSlug, isEditor, storeDiscounts, children, onSelect, onDelete, selectedId }) {
    return (
        <React.Suspense fallback={<WidgetSkeleton type={type} />}>
            {(() => {
                switch (type) {
                    ...
                    case 'product_grid':
                        return <ProductGridRenderer settings={settings} products={products} viewMode={viewMode} store={store} isEditor={isEditor} storeDiscounts={storeDiscounts} categories={categories} categorySlug={categorySlug} parentSlug={parentSlug} childSlug={childSlug} />;
