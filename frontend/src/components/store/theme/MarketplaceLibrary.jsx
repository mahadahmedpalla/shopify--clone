    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {themes.map(item => {
                    // Safety check if theme is null (e.g., deleted or RLS restricted)
                    if (!item.theme) return null;

                    const isDraft = item.theme.status === 'draft';

                    return (
                        <Card key={item.id} className={`overflow-hidden border-2 transition-all ${item.is_active ? 'border-indigo-500 shadow-md bg-indigo-50/10' : 'border-slate-200 hover:border-slate-300'} ${isDraft ? 'opacity-75' : ''}`}>
                            <div className="aspect-video bg-slate-100 flex items-center justify-center text-slate-300 relative">
                                ...
                                {isDraft && (
                                    <div className="absolute top-2 left-2 bg-amber-500 text-white text-[10px] font-bold px-2 py-1 rounded-full flex items-center shadow-sm">
                                        DRAFT
                                    </div>
                                )}
                            </div>
                            <div className="p-4">
                                ...
                                <div className="flex space-x-2">
                                    <Button
                                        ...
                                        className="flex-1"
                                        disabled={isDraft}
                                        title={isDraft ? "This theme is currently in draft and cannot be applied." : ""}
                                    >
