...
const handleToggleStatus = async (page) => {
    if (isExpired && !page.is_published) {
        alert("Store Terminated/Expired. Please renew your store subscription in Settings to publish pages.");
        return;
    }
            ...
            <div className="mb-8 bg-red-50 border-l-4 border-red-500 p-4 rounded-r-lg shadow-sm animate-pulse">
                <div className="flex items-start">
                    ...
                    <div className="flex-1">
                        <h3 className="text-sm font-bold text-red-800">Store Subscription Expired</h3>
                        <p className="text-sm text-red-700 mt-1">
                            Your store is currently inactive. You cannot publish pages or make them visible to customers explicitly.
                        </p>
                        <button
                            onClick={() => navigate(`/store/${storeId}/settings`)}
                            className="mt-3 text-sm font-semibold text-red-800 underline hover:text-red-900"
                        >
                            Renew Subscription in Settings &rarr;
                        </button>
                    </div>
                </div>
            </div>
